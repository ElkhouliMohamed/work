...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            <h5 class="text-sm font-medium text-gray-600">Level</h5>
            <p class="text-2xl font-semibold text-gray-900 mt-2">{{ $level }}</p>
        </div>
        <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            <h5 class="text-sm font-medium text-gray-600">Payment Requests</h5>
            <p class="text-2xl font-semibold text-gray-900 mt-2">{{ $stats['payment_requested'] ?? 0 }}</p>
        </div>
    </div>
        ...
        <form method="GET" class="flex flex-col md:flex-row gap-4 items-end">
            <div class="flex-1">
                ...
                <select name="status" id="status"
                    ...
                    <option value="En Attente" {{ request('status')==='En Attente' ? 'selected' : '' }}>Pending
                    </option>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        ...
        @endif
        @if (session('warning'))
        <div class="bg-yellow-50 border-l-4 border-yellow-400 text-yellow-700 p-4 mb-4">{{ session('warning') }}
        </div>
        @endif
        ...
        <div class="p-6 border-b border-gray-200 flex justify-between items-center">
            ...
            <div class="space-x-2">
                ...
                <form action="{{ route('commissions.demandePaiementAll') }}" method="POST">
                    @csrf
                    <button class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700"
                        type="submit">Demander paiement pour toutes</button>
                </form>
                @endif
            </div>
        </div>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm space-x-2">
                            ...
                            <button onclick="requestPayment({{ $commission->id }})"
                                class="text-blue-600 hover:text-blue-900 font-medium">
                                Request Payment
                            </button>
                            @endif
                            @if ($commission->statut === 'Payé' && $commission->payment_proof_path)
                            <a href="{{ Storage::url($commission->payment_proof_path) }}"
                                class="text-green-600 hover:text-green-900 font-medium" target="_blank">
                                Payment Proof
                            </a>
                            @endif
                            @endif
    ...
    <div class="relative top-20 mx-auto p-6 border w-full max-w-md shadow-lg rounded-lg bg-white">
        <form id="paymentForm" method="POST" action="" enctype="multipart/form-data">
            ...
            <div class="space-y-4">
                ...
                <div>
                    <label for="payment_proof" class="block text-sm font-medium text-gray-700">Payment
                        Proof</label>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
                ...
                window.requestPayment = function(commissionId) {
                    if (confirm('Are you sure you want to request payment for this commission?')) {
                        window.location.href = `{{ route('commissions.requestPayment', ':id') }}`.replace(':id', commissionId);
                    }
                };

                // Clear Commission Confirmation
