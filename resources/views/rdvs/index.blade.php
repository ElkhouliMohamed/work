...
<div class="container mx-auto px-4 py-8 sm:px-6 lg:px-8">
    ...
    <form method="GET" action="{{ route('rdvs.index') }}" class="mb-6 bg-white rounded-lg shadow-sm p-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="flex space-x-2">
                <select name="status"
                    class="block w-full pl-3 pr-10 py-2 text-base border border-gray-200 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div
            class="relative p-4 rounded-lg shadow-sm bg-gradient-to-r from-blue-50 to-blue-100 border-l-4 border-blue-500 overflow-hidden">
            <div class="text-sm font-medium text-blue-800">Total RDV</div>
            <div class="text-2xl font-bold text-gray-800">{{ $totalCount }}</div>
            <svg class="absolute right-4 top-4 h-8 w-8 text-blue-500 opacity-20" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div
            class="relative p-4 rounded-lg shadow-sm bg-gradient-to-r from-green-50 to-green-100 border-l-4 border-green-500 overflow-hidden">
            <div class="text-sm font-medium text-green-800">À venir</div>
            <div class="text-2xl font-bold text-gray-800">{{ $upcomingCount }}</div>
            <svg class="absolute right-4 top-4 h-8 w-8 text-green-500 opacity-20" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div
            class="relative p-4 rounded-lg shadow-sm bg-gradient-to-r from-purple-50 to-purple-100 border-l-4 border-purple-500 overflow-hidden">
            <div class="text-sm font-medium text-purple-800">Confirmés</div>
            <div class="text-2xl font-bold text-gray-800">{{ $confirmedCount }}</div>
            <svg class="absolute right-4 top-4 h-8 w-8 text-purple-500 opacity-20" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div
            class="relative p-4 rounded-lg shadow-sm bg-gradient-to-r from-gray-50 to-gray-100 border-l-4 border-gray-500 overflow-hidden">
            <div class="text-sm font-medium text-gray-800">Terminés</div>
            <div class="text-2xl font-bold text-gray-800">{{ $completedCount }}</div>
            <svg class="absolute right-4 top-4 h-8 w-8 text-gray-500 opacity-20" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div
                                    ...
                                    <span class="text-blue-600 font-medium">{{ substr($rdv->contact->prenom, 0, 1) }}{{
                                        substr($rdv->contact->nom, 0, 1) }}</span>
                                </div>
                                <div class="ml-4">
                                    <div class="text-sm font-medium text-gray-900">{{ $rdv->contact->prenom }}
                                        {{ $rdv->contact->nom }}</div>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="flex-shrink-0 mr-3">
                                    <div
                                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $rdv->date->isToday() ? 'bg-blue-100 text-blue-800' : ($rdv->date->isPast() ? 'bg-gray-100 text-gray-800' : 'bg-green-100 text-green-800') }}">
                                        {{ $rdv->date->format('d M') }}
                                    </div>
                                </div>
                                <div>
                                    <div class="text-sm font-medium text-gray-900">{{ $rdv->date->format('H:i') }}
                                    </div>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            @endphp
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium {{ $typeColor }}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 p-1">
                                {{ ucfirst($rdv->freelancer->name) }}
                            </span>
                        </td>
                        <!-- Status Column -->
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            $statusColor =
                            $statusColors[strtolower($rdv->statut)] ?? 'bg-gray-100 text-gray-800';
                            @endphp
                            <span
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <div
                                class="devis-item text-xs flex items-center {{ $devis->statut == 'Accepté' ? 'text-green-600' : 'text-gray-600' }}">
                                <span class="font-medium">{{ $devis->statut }}</span>
                                <span class="text-xs ml-2">{{ number_format($devis->montant, 2) }}
                                    MAD</span>
                            </div>
                            @endforeach
                            @else
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right">
                            <div class="flex justify-end space-x-1">
                                ...
                                <a href="{{ route('devis.create', ['rdv' => $rdv->id]) }}"
                                    class="p-2 rounded-md inline-flex items-center justify-center bg-blue-100 text-blue-100 text-blue-600 hover:bg-blue-200 transition-colors duration-200"
