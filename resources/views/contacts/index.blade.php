...
<div class="container mx-auto px-4 py-8 sm:px-6 lg:px-8">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Téléphone</th>
                    ...
                    <tr class="hover:bg-gray-50 transition-all duration-150 ease-in-out">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="flex-shrink-0 h-10 w-10 rounded-full bg-indigo-100 flex items-center justify-center shadow-sm">
                                    <span class="text-indigo-600 font-semibold text-lg">{{ strtoupper(substr($contact->nom, 0, 1)) }}{{ strtoupper(substr($contact->prenom, 0, 1)) }}</span>
                                </div>
                                <div class="ml-4">
                                    <div class="text-sm font-medium text-gray-900">{{ $contact->nom }} {{ $contact->prenom }}</div>
                                    <div class="text-xs text-gray-500">{{ $contact->nom_entreprise ?? 'Sans entreprise' }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex items-center space-x-3">
                                ...
                                <form action="{{ route('contacts.restore', $contact->id) }}" method="POST" class="inline-block">
                                    ...
                                    @method('PUT')
                                    <button type="submit"
                                            class="text-green-600 hover:text-green-800 p-1 rounded-full hover:bg-green-100 transition-all duration-200 ease-in-out"
                                            title="Restaurer">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                ...
                                <a href="{{ route('contacts.show', $contact) }}"
                                   class="text-gray-600 hover:text-gray-800 p-1 rounded-full hover:bg-gray-100 transition-all duration-200 ease-in-out"
                                   title="Voir détails">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
    ...
    <div class="mt-6 px-4 py-3 flex items-center justify-between border-t border-gray-200 bg-white rounded-b-lg shadow-sm sm:px-6">
